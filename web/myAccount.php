...
<?php
	...
	if (empty($_SESSION['aid'])) {
		...
		echo '<p class="auto-style4">Your Transactions:</p>
			<table style="width: 100%">
				<tr>
					<td class="auto-style6" width="100"><strong>Seller</strong></td>
					<td class="auto-style6" width="100"><strong>Tracking</strong></td>
					<td class="auto-style6" width="100"><strong>Sale Date</strong></td>
					<td class="auto-style6" width="100"><strong>Paid With</strong></td>
					<td class="auto-style6" width="100"><strong>Ships To</strong></td>
					<td class="auto-style6" width="100"><strong>Ships From</strong></td>
				</tr>';
		$query2 = "SELECT T.seller, T.tracking_number, T.date_of_sale, T.paid_with, A1.street street_to, A1.city city_to, A1.state state_to, A1.zip zip_to, A2.street street_from, A2.city city_from, A2.state state_from, A2.zip zip_from FROM Transactions T, Addresses A1, Addresses A2 WHERE T.buyer = \"" . $_SESSION['aid'] . "\"" . " AND T.ships_to = A1.address_id AND T.ships_from = A2.address_id";

		$rs2 = mysql_query($query2);

		while ($row2 = mysql_fetch_assoc($rs2)) {
			echo "<tr>";
			echo "<td class=\"auto-style5\">" . $row2['seller'] . "</td>" .
